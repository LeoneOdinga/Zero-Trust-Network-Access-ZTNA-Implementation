...
# System processes that should NEVER be flagged as IOCs even if their
# command-line arguments happen to contain an IOC keyword.
# e.g. `journalctl _COMM=nmap` or `sh -c "grep mimikatz /var/log/..."`
# These are the agent's own scanning subprocesses or legitimate system tools.
SCAN_EXCLUSIONS = {
    "sh", "bash", "dash", "zsh", "ksh", "fish",     # shells
    "python", "python3", "python2", "perl", "ruby",  # interpreters
    "journalctl", "systemctl", "systemd",            # systemd
    "ausearch", "aureport", "auditctl",              # audit tools
    "grep", "egrep", "fgrep", "awk", "sed",          # text tools
    "find", "xargs", "cat", "tail", "head",          # file tools
    "sudo", "su", "doas",                            # privilege tools
    "ssh", "sshd",                                   # ssh (not psexec)
    "ps", "top", "htop", "kill", "pkill",            # process tools
}
...
def _scan_live_processes() -> list:
    ...
    Matching rules:
    - Match against process NAME only (not full cmdline arguments).
      Checking cmdline arguments causes false positives — legitimate
      system tools like journalctl, grep, sh etc. can have IOC names
      as arguments without actually being IOC tools themselves.
    - Skip processes in SCAN_EXCLUSIONS regardless of their arguments.
    - Exception: for interpreter processes (python3, perl etc.), also
      check argv[1] (the script being run) since tools like impacket
      are often invoked as `python3 secretsdump.py`.
    """
    INTERPRETERS = {"python", "python3", "python2", "perl", "ruby", "node"}
    found = []
    if HAS_PSUTIL:
        for proc in psutil.process_iter(["name", "pid", "cmdline"]):
            try:
                name    = (proc.info.get("name") or "").lower()
                cmdlist = proc.info.get("cmdline") or []

                # Skip excluded system processes
                if name in SCAN_EXCLUSIONS:
                    continue

                # Check process name
                if any(k in name for k in IOC_LIST):
                    found.append(
                        f"{proc.info['name']} (PID {proc.info['pid']})"
                    )
                    continue

                # For interpreters, also check the script name (argv[1])
                if name in INTERPRETERS and len(cmdlist) > 1:
                    script = os.path.basename(cmdlist[1]).lower()
                    if any(k in script for k in IOC_LIST):
                        found.append(
                            f"{proc.info['name']}:{os.path.basename(cmdlist[1])}"
                            f" (PID {proc.info['pid']})"
                        )
    ...
    try:
        ...
        for pid in os.listdir("/proc"):
            ...
            try:
                ...
                # Parse null-delimited argv — only check argv[0] (the
                # executable name) and argv[1] (script for interpreters).
                # Do NOT scan all arguments — they may contain IOC names
                # innocuously (e.g. journalctl _COMM=nmap).
                argv = [
                    a.decode("utf-8", errors="replace")
                    for a in raw_cmdline.split(b"\x00")
                    if a
                ]
                if not argv:
                    continue
                ...
                # Skip excluded system processes
                if exe_name in SCAN_EXCLUSIONS:
                    continue

                # Check executable name
                matched_ioc = next(
                    (ioc for ioc in IOC_LIST if ioc in exe_name), None
                )

                # For interpreters also check the script name (argv[1])
                if not matched_ioc and len(argv) > 1:
                    INTERPRETERS = {"python", "python3", "python2",
                                    "perl", "ruby", "node"}
                    if exe_name in INTERPRETERS:
                        script_name = os.path.basename(argv[1]).lower()
                        matched_ioc = next(
                            (ioc for ioc in IOC_LIST if ioc in script_name),
                            None
                        )

                if matched_ioc:
                    _add(f"{matched_ioc} (PID {pid})", "proc-recent")
